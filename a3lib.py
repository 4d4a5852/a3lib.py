                ...
                for info in p.infolist():
                    if (fnmatch.fnmatch(info.filename.decode().lower(),
                        ...
                        with p.open(info) as src:
                            dst_name = src.name.decode().replace('\\',
                                                                 os.path.sep)
...
def main():
    ...
    parser_key.add_argument('--privout', action='store_true', default=False,
                            help='write .biprivatekey')
    parser_key.add_argument('--pubin', action='store_true', default=False,
                            help='public key as input')
    parser_key.add_argument('--pubout', action='store_true', default=False,
                            help='write .bikey')
    parser_key.add_argument('--keyform', default='bi',
                            choices=['bi', 'der', 'pem'],
                            help='format of the input key - default: bi')
    ...
    parser_verify = subparsers.add_parser('verify',
                                          help='verify the signature of a PBO')
    ...
    parser_verify.add_argument('--keyform', default='bi',
                               choices=['bi', 'der', 'pem'],
                               help='Format of the key - default: bi')
    parser_verify.add_argument('--privin', action='store_true', default=False,
                               help='private key as input')
    parser_verify.set_defaults(func=_verify)
    # create the parser for the "bisign" command
    parser_bisign = subparsers.add_parser('bisign', help=
                                          'print bisign/extract public key')
    parser_bisign.add_argument('--pubout', action='store_true', default=False,
                               help='extract public key from signature')
    ...
    parser_pbo = subparsers.add_parser('pbo', help='create/extract/list PBOs')
    pbo_mode_group = parser_pbo.add_mutually_exclusive_group(required=True)
    pbo_mode_group.add_argument('-c', '--create', action='store_true',
                                default=False, help='create a new pbo file')
    pbo_mode_group.add_argument('-i', '--info', action='store_true',
                                default=False,
                                help='print information about the pbo file')
    pbo_mode_group.add_argument('-l', '--list', action='store_true',
                                default=False,
                                help='list the content of the pbo file')
    pbo_mode_group.add_argument('-x', '--extract', action='store_true',
                                default=False, help='extract a pbo file')
    parser_pbo.add_argument('-f', '--file', required=True, help='pbo file',
                            metavar='PBO')
    parser_pbo.add_argument('files', default=[], help='files to be added',
                            nargs='*', metavar='FILE')
    parser_pbo.add_argument('--include', default='*',
                            help='include filter pattern')
    parser_pbo.add_argument('--exclude', default='',
                            help='exclude filter pattern')
    parser_pbo.add_argument('-e', '--header_extension', default=[],
                            action='append',
                            help='header extension to be added', nargs=2,
                            metavar=('NAME', 'VALUE'))
    parser_pbo.add_argument('--no-pboprefixfile', dest='pboprefixfile',
                            action='store_false', default=True,
                            help='don\'t use a $PBOPREFIX$ file')
    parser_pbo.add_argument('--no-recursion', dest='recursion',
                            action='store_false', default=True, help="""don't
                            automatically ascend into directories when adding
                            files""")
    parser_pbo.add_argument('--timestamps', dest='update_timestamps',
                            action='store_true', default=False,
                            help='update timestamps when extracting PBOs')
